 ...
 * pool results); `?board=battles` selects the Battle Elo ladder (M16);
 * no/invalid params fall back to the global board. Composition lives in the
 * app layer: pick the read model from the query, hand the view to the slice's
 * table.
 */
export default async function LeaderboardPage({
  searchParams,
}: {
  searchParams: Promise<{ role?: string; board?: string }>;
}) {
  const identity = await getIdentity();
  if (!identity) redirect('/');
  if (identity.status !== 'complete') redirect('/onboarding');

  const { role, board } = await searchParams;
  const view =
    board === 'battles'
      ? await getEloLeaderboard(identity.userId)
      : role && isJobRole(role)
